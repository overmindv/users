...
func Load() (Config, error) {
	...
	cfg := Config{
		HTTP: HTTP{
			Address: ":" + port, ReadTimeout: envDuration("HTTP_READ_TIMEOUT", 5*time.Second),
			WriteTimeout: envDuration("HTTP_WRITE_TIMEOUT", 15*time.Second), ShutdownTimeout: envDuration("SHUTDOWN_TIMEOUT", 10*time.Second),
		},
		Database: Database{
			DSN:            env("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/arcee?sslmode=disable"),
			MaxConnections: int32(envInt("DB_MAX_CONNS", 20)), MinConnections: int32(envInt("DB_MIN_CONNS", 2)),
			MaxConnLifetime: envDuration("DB_MAX_CONN_LIFETIME", 30*time.Minute),
		},
		JWT: JWT{Secret: env("JWT_SECRET", "local-development-secret-change-me"), Issuer: env("JWT_ISSUER", "arcee"), Lifetime: envDuration("JWT_TTL", 24*time.Hour)},
	}
